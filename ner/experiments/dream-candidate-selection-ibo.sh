...
DREAM_STRATEGIES=( random certainty mix )
...
OUTPUT=$OUT_DIR/candi_${AL_STRATEGIES[$al_idx]}_${DREAM_STRATEGIES[$dr_idx]}_${DATASET_NAME}_${EXP_NAME}_${DATE}
mkdir -p $OUTPUT
...
cd $SRC_PATH && python AL-crf-dreaming.py --root_dir $ROOT_DIR --dataset_name $DATASET_NAME  \
    ...
    --al_candidate_selection_mode ${AL_STRATEGIES[$al_idx]} \
    --dreaming_candidate_selection_mode ${DREAM_STRATEGIES[$dr_idx]}
rm -r -f $CACHE_PATH