...
class CRFTagger(object):
    ...
    def __init__(self, modelfile, num_classes=5):
        # print("CRF Tagger")
    ...
    def get_predictions(self, sent):
        ...
        for i in range(len(x)):
            y_i = []
            for y in range(1, self.num_classes + 1):
    ...
    def test(self, test_sents, label2str):
        ...
        for i in range(len(Y_true)):
            for j in range(len(Y_true[i])):
                ...
                if y_pred[i][j] == Y_true[i][j]:
                    corr += 1
                if label2str[int(y_pred[i][j])] != 'O':  # not 'O'
                    pre_tot += 1
                    if y_pred[i][j] == Y_true[i][j]:
                        pre += 1
                if label2str[int(Y_true[i][j])] != 'O':
                    rec_tot += 1
                    if y_pred[i][j] == Y_true[i][j]:
                        rec += 1
    ...
    def predict(self, sents):
        X_features = [self.sent2features(s) for s in sents]
        tagger = pycrfsuite.Tagger()
        tagger.open(self.modelfile)
